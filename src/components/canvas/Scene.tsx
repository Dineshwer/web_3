'use client';

import { Canvas } from '@react-three/fiber';
import { Suspense, useRef } from 'react';
import { OrbitControls, PerspectiveCamera, Environment, Stars } from '@react-three/drei';
import { Physics } from '@react-three/cannon';
import { EffectComposer, Bloom, Vignette } from '@react-three/postprocessing';
import { GodRays } from '@react-three/postprocessing';
import { useStore } from '@/store/useStore';
import * as THREE from 'three';

import Globe from './Globe';
import FloatingData from './FloatingData';
import CameraRig from './CameraRig';

// Placeholder for a loading state component
function Loader() {
    return null;
}

export default function Scene() {
    const { activeView } = useStore();
    const enableOrbit = activeView === 'OVERVIEW';
    const lightSourceRef = useRef<THREE.Mesh>(null);

    return (
        <Canvas
            shadows
            dpr={[1, 2]}
            gl={{ antialias: false, stencil: false, depth: true }}
            camera={{ position: [0, 0, 12], fov: 45 }}
        >
            <CameraRig />
            <PerspectiveCamera makeDefault position={[0, 0, 14]} fov={50} />

            <OrbitControls
                makeDefault
                enableZoom={true}
                maxDistance={20}
                minDistance={5}
                enabled={enableOrbit}
                enableDamping={true}
                autoRotate={activeView === 'OVERVIEW'}
                autoRotateSpeed={0.5}
            />

            <color attach="background" args={['#050505']} />
            <fog attach="fog" args={['#050505', 5, 30]} />

            <ambientLight intensity={0.1} />
            <spotLight position={[10, 10, 10]} angle={0.5} penumbra={1} intensity={0.8} color="#00f3ff" castShadow />
            <pointLight position={[-10, -5, -10]} intensity={0.5} color="#ff2a2a" />

            <Stars radius={100} depth={50} count={5000} factor={4} saturation={0} fade speed={1} />

            {/* Light source for god rays - dimmed */}
            <mesh ref={lightSourceRef} position={[0, 0, 0]}>
                <sphereGeometry args={[0.3, 16, 16]} />
                <meshBasicMaterial color="#00f3ff" opacity={0.3} transparent />
            </mesh>

            <Physics gravity={[0, 0, 0]}>
                <Suspense fallback={<Loader />}>
                    <Environment preset="city" />
                    <Globe />
                    <FloatingData count={30} />
                </Suspense>
            </Physics>

            <EffectComposer>
                <Bloom luminanceThreshold={0.3} mipmapBlur intensity={0.8} radius={0.3} />
                <Vignette eskil={false} offset={0.1} darkness={1.1} />
                <GodRays
                    sun={lightSourceRef}
                    samples={30}
                    density={0.98}
                    decay={0.96}
                    weight={0.2}
                    exposure={0.15}
                    clampMax={1}
                />
            </EffectComposer>
        </Canvas>
    );
}
